        ...
        <div>
            ...
            <div className='flex flex-wrap justify-center gap-5 '>
                {Object.entries(expeditionData.expeditions).map((expedition,i) => (
                    ...
                    // </div>
                    <motion.div onClick={e=>{e.preventDefault();handleClick(expedition[0])}} whileHover={{scale:1.1}} initial={{opacity:0, x:-100}} whileInView={{opacity:1,x:0}} transition={{delay:(i+0.01)/10, duration:0.5}} viewport={{once:true}} key={expedition.id} className="max-w-sm rounded overflow-hidden shadow-lg m-4 relative group" style={{ width: '100%', height: '500px' }}>
